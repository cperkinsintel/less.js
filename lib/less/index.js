...
less.Parser.set_importer = function(userfs)
{
    ...
    less.Parser.importer = function (file, currentFileInfo, callback, env) {
        ...
        if(isUrl || isUrlRe.test(currentFileInfo.currentDirectory)) {
            ...
            var fix_path = function(some_path)
            {                                                       //   C:\this\is\the\way
                var fixed_1 = some_path.replace(/\\/g, "/");        //   C:/this/is/the/way
                var fixed_2 = fixed_1.replace(/(.):/, "/$1" );      //   /C/this/is/the/way
                return fixed_2;
            };
            var path_join = userfs ? function(a, b){ return fix_path(path.join(a, b)); } : path.join;
            var pathnames = paths.map(function(p){ return path_join(p, file);});


            var method = !userfs ? fs.exists : function(path, callback){ userfs.exists(path).then(callback).fail(function(er){return false; })};
            async.detect(pathnames, method, function(pname) // result now equals the first file in the list that exists (or null)
            {
                pathname = pname;
                //console.log("found a pathname", pathname);
                if (!pathname) {
                ...
                else
                {
                    if(userfs)
                    {
                        userfs.readFile(pathname)
                            .fail(function(err){ callback({type:"File", message:"readFile failed " + pathname + " " +  JSON.stringify(err)}); })
                            .then(function(data){  parseFile(null, data.toString()); })
                            .fail(function(err){ callback({type:"File", message:"parseFile failed " + err.message}); });
