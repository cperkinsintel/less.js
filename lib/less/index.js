...
less.Parser.set_importer = function(userfs)
{
    ...
    less.Parser.importer = function (file, currentFileInfo, callback, env) {
        ...
        if(false)// (isUrl || isUrlRe.test(currentFileInfo.currentDirectory)) {
            ...
            console.log("LESS - PATHNAMES", pathnames);

            var method = !userfs ? fs.exists : function(path, callback){ userfs.exists(path).then(callback); };
