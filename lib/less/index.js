...
less.Parser.set_importer = function(userfs)
{
    ...
    less.Parser.importer = function (file, currentFileInfo, callback, env) {
        ...
        if (isUrl || isUrlRe.test(currentFileInfo.currentDirectory)) {
            ...
            var pathnames = paths.map(function(p){ return path.join(p, file);});
            ...
            async.detect(pathnames, method, function(pname) // result now equals the first file in the list that exists (or null)
            {
                ...
                else
                {
                    if(userfs)
                    {
                        userfs.exists(pathname)
                            .then(function(exists)
                            {
                                if(!exists){ throw new Error("file " + file + "wasn't found");  }
                                return userfs.stat(pathname);
                            })
                            .then(function(stat)
                            {
                                if(!stat.isFile){ throw new Error(file + "not a file");}
                                return userfs.readFile(pathname);
                            })
                            .then(function(data){ parseFile(null, data); })
                            .fail(function(err){ callback({type:"File", message:err.message}); });
                    }
                    else
                    {
