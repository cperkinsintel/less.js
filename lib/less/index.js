...
less.Parser.set_importer = function(userfs)
{
    ...
    less.Parser.importer = function (file, currentFileInfo, callback, env) {
        ...
        if(isUrl || isUrlRe.test(currentFileInfo.currentDirectory)) {
            ...
            var method = !userfs ? fs.exists : function(path, callback){ userfs.exists(path).then(callback).fail(function(er){ callback({type:"File", message:"path does not exist " + path + er.message}); })};
